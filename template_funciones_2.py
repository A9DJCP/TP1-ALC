...
def metpot1(A,tol=1e-8,maxrep=np.Inf):
   ...
   while (np.abs(l) != 0) and np.abs(l1-l)/np.abs(l) > tol and nrep < maxrep: # Si estamos por debajo de la tolerancia buscada 
      ...
      l1 = v1.T @ (A @ v1) # Calculo autovalor
...
def metpotI(A,mu,tol=1e-8,maxrep=np.Inf):
    ...
    # Para esto tenemos que calcular la inversa de A + mu * I.
    if(mu == 0): # Si mu es cero entonces L + muI = L y no tiene inversa (porque tiene autovalor 0 con el autovector de unos).
        return np.ones(A.shape[0], 0, True)
...
def laplaciano_iterativo(A,niveles,nombres_s=None):
    ...
    if A.shape[0] == 1 or niveles == 0: # Si llegamos al último paso, retornamos los nombres en una lista
        ...
        v = np.sign(v)
        ind_pos = np.where(v == 1)[0] # Los miembros del primer grupo
        ind_neg = np.where(v == -1)[0] # Los miembros del segundo grupo
        Ap = A[np.ix_(ind_pos, ind_pos)] # Asociado al signo positivo
        Am = A[np.ix_(ind_neg, ind_neg)] # Asociado al signo negativo
